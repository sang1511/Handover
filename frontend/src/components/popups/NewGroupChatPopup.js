...
const NewGroupChatPopup = ({ open, onClose, onCreate, loading }) => {
  ...
  const handleSubmit = (e) => {
    ...
    onCreate(groupName, selectedUsers.map(u => u._id));
  };
  ...
  return (
    <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', background: 'rgba(0,0,0,0.2)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <form onSubmit={handleSubmit} style={{ background: '#fff', padding: 28, borderRadius: 16, minWidth: 380, boxShadow: '0 4px 24px rgba(0,0,0,0.18)', position: 'relative', maxWidth: 420, width: '100%' }}>
        <h4 style={{margin:'0 0 18px 0'}}>Tạo nhóm chat mới</h4>
        <input
          type="text"
          placeholder="Tên nhóm..."
          value={groupName}
          onChange={e => setGroupName(e.target.value)}
          style={{ width: '100%', marginBottom: 14, padding: 10, borderRadius: 8, border: '1.5px solid #e0e7ef', fontSize: 16, outline: 'none', background: '#f8fafc' }}
          disabled={loading}
          autoFocus
        />
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10 }}>
          ...
          <button type="submit" disabled={loading || !groupName || selectedUsers.length === 0} style={{padding:'8px 18px', borderRadius:8, border:'none', background:'#ff6a5b', color:'#fff', fontWeight:600, cursor: loading || !groupName || selectedUsers.length === 0 ? 'not-allowed' : 'pointer'}}>Tạo nhóm</button>
        </div>
      </form>
    </div>
  );
...
export default NewGroupChatPopup; 